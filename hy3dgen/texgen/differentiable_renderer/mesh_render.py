...
from .camera_utils import transform_pos, get_mv_matrix, get_orthographic_projection_matrix, get_perspective_projection_matrix
...
class MeshRender():
    def __init__(
        ...
        self.bake_unreliable_kernel_size = int((2 / 512) * max(self.default_resolution[0], self.default_resolution[1]))
        ...
        if self.raster_mode == 'cr':
            ...
            raise Exception(f"No raster named {self.raster_mode}")
        ...
        if camera_type == 'orth':
            ...
            raise Exception(f"No camera type {camera_type}")
    ...
    def raster_texture(self, tex, uv, uv_da=None, mip_level_bias=None, mip=None, filter_mode='auto', boundary_mode='wrap', max_mip_level=None):

        if self.raster_mode == 'cr':
            raise Exception(f"Texture is not implemented in cr.")
        else:
            raise Exception(f"No raster named {self.raster_mode}.")
    ...
    def _render(self, glctx, mvp, pos, pos_idx, uv, uv_idx, tex, resolution, max_mip_level, keep_alpha, filter_mode):
        pos_clip = transform_pos(mvp, pos)
        if isinstance(resolution, (int, float)):
            resolution = [resolution, resolution]
        rast_out, rast_out_db = self.raster_rasterize(glctx, pos_clip, pos_idx, resolution=resolution)

        tex = tex.contiguous()
        if filter_mode == 'linear-mipmap-linear':
            texc, texd = self.raster_interpolate(uv[None, ...], rast_out, uv_idx, rast_db=rast_out_db, diff_attrs='all')
            color = self.raster_texture(tex[None, ...], texc, texd, filter_mode='linear-mipmap-linear', max_mip_level=max_mip_level)
        else:
    ...
    def render(
        self, elev, azim, camera_distance=None, center=None, resolution=None, tex=None, keep_alpha=True, bgcolor=None, filter_mode=None, return_type='th',
    ):

        proj = self.camera_proj_mat
        r_mv = get_mv_matrix(
            elev=elev, azim=azim, camera_distance=self.camera_distance if camera_distance is None else camera_distance, center=center,
        )
        ...
        image = self._render(
            None, r_mvp, self.vtx_pos, self.pos_idx, self.vtx_uv, self.uv_idx, self.tex if tex is None else tex,
            self.default_resolution if resolution is None else resolution, self.max_mip_level, True,
    ...
    def render_normal(
        self, elev, azim, camera_distance=None, center=None, resolution=None, bg_color=[1, 1, 1], use_abs_coor=False, normalize_rgb=True, return_type='th',
    ):
        ...
        vertex_normals = trimesh.geometry.mean_vertex_normals(
            vertex_count=self.vtx_pos.shape[0], faces=self.pos_idx.cpu(), face_normals=face_normals.cpu(),
        )
    ...
    def render_depth(self, elev, azim, camera_distance=None, center=None, resolution=None, return_type='th'):
        pos_camera, pos_clip = self.get_pos_from_mvp(elev, azim, camera_distance, center)
    ...
    def back_project(self, image, elev, azim, camera_distance=None, center=None, method=None):
        ...
        if method == 'linear':
            ...
            raise Exception(f"No bake mode {method}")
