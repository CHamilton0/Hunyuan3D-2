...
def timestep_embedding(t: torch.Tensor, dim: int, max_period=10000.0, time_factor=1000.0):
    """
...
class SelfAttention(nn.Module):
    def __init__(self, dim: int, num_heads=8, qkv_bias=False):
        super().__init__()
...
class DoubleStreamBlock(nn.Module):
    def __init__(self, hidden_size: int, num_heads: int, mlp_ratio: float, qkv_bias=False):
        super().__init__()
...
class SingleStreamBlock(nn.Module):
    ...
    def __init__(self, hidden_size: int, num_heads: int, mlp_ratio=4.0, qk_scale: float = None):
        super().__init__()
...
class Hunyuan3DDiT(nn.Module):
    def __init__(
        self, in_channels=64, context_in_dim=1536, hidden_size=1024, mlp_ratio=4.0, num_heads=16, depth=16, depth_single_blocks=32, axes_dim: list[int] = [64],
        theta=10_000, qkv_bias=True, time_factor=1000.0, guidance_embed=False, ckpt_path: str = None, **kwargs,
    ):
        super().__init__()
