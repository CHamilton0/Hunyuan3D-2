...
def _gen_shape(
    caption=None, image=None, mv_image_front=None, mv_image_back=None, mv_image_left=None, mv_image_right=None, steps=50, guidance_scale=7.5,
            ...
            with gr.Column(scale=3):
                ...
                with gr.Tabs(selected='tab_options' if TURBO_MODE else 'tab_export'):
                    with gr.Tab("Options", id='tab_options', visible=TURBO_MODE):
                        gen_mode = gr.Radio(
                            label="Generation Mode", info="Recommendation: Turbo for most cases, Fast for very complex cases, Standard seldom use.",
                            choices=['Turbo', 'Fast', 'Standard'], value='Turbo',
                        )
                        decode_mode = gr.Radio(
                            label="Decoding Mode", info="The resolution for exporting mesh from generated vectset.",
                            choices=['Low', 'Standard', 'High'], value='Standard',
                        )
            ...
            with gr.Column(scale=3 if MV_MODE else 2):
                ...
                with gr.Tabs(selected='tab_img_gallery') as gallery:
                    ...
                    with gr.Tab("Text to 3D Gallery", id='tab_txt_gallery', visible=HAS_T2I and not MV_MODE) as tab_gt:
                        with gr.Row():
                            gr.Examples(examples=example_ts, inputs=[caption], label=None, examples_per_page=18)
